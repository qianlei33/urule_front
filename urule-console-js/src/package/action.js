...
export function refreshKnowledgeCache(project,packageId,files) {
    $.ajax({
        ...
        success:function (data) {
            ...
            if(info){
                bootbox.confirm(`<h3 class="text-danger">发布操作成功！</h3>产品[${project}]中配置了如下客户端：<div class="well" style="margin: 5px 0px 5px 0px">${info}</div>是否将此次更新的知识包推送到这些客户端中？`,function (result) {
                    if(!result){
                        return;
                    }
